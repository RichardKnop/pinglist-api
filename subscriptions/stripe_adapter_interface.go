...
type StripeAdapterInterface interface {
	...
	CreateSubscription(customerID, planID string, trialEnd int64) (*stripe.Sub, error)
	GetSubscription(subscriptionID, customerID string) (*stripe.Sub, error)
	UpdateSubscription(subscriptionID, customerID, planID string) (*stripe.Sub, error)
