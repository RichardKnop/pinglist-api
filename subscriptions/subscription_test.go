...
func (suite *SubscriptionsTestSuite) TestFindSubscriptionByID() {
	...
	if assert.NotNil(suite.T(), subscription) {
		...
		assert.Equal(suite.T(), suite.cards[0].ID, subscription.Card.ID)
	}
}
...
func (suite *SubscriptionsTestSuite) TestFindSubscriptionByCardID() {
	var (
		subscription *Subscription
		err          error
	)

	// When we try to find a subscription with a bogus card ID
	subscription, err = suite.service.FindSubscriptionByCardID(12345)
	...
	// When we try to find a plan with a valid card ID
	subscription, err = suite.service.FindSubscriptionByCardID(suite.subscriptions[0].Card.ID)
