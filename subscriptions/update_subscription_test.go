...
func (suite *SubscriptionsTestSuite) TestUpdateSubscriptionWithoutPermission() {
	// Prepare a request
	r, err := http.NewRequest(
		"PUT",
		fmt.Sprintf("http://1.2.3.4/v1/subscriptions/%d", suite.subscriptions[0].ID),
		nil,
	)
	assert.NoError(suite.T(), err, "Request setup should not get an error")
	...
	expectedJSON, err := json.Marshal(
		map[string]string{"error": ErrUpdateSubscriptionPermission.Error()})
